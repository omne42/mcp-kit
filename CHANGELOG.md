...
- `pm-mcp-kit`：`mcp.json`（v1）解析、MCP server 连接与连接缓存管理（`Config/Manager/Connection`）。
...
- `pm_mcp_kit::mcp`：常用 MCP methods 的轻量 typed wrapper 子集（参考 `example/codex/codex-rs/mcp-types`）。
...
### Changed
- `Config::load` 默认路径发现：优先 `./.mcp.json` / `./mcp.json`，再回退到 `./.codepm_data/spec/mcp.json`。
- `mcpctl` 现在需要 `--features cli` 构建（避免 library 依赖方被迫引入 clap）。
- `pm-mcp-kit::Manager` 默认 `TrustMode::Untrusted`：拒绝 `transport=stdio|unix`；`streamable_http` 仅允许 `https` 且非 localhost/私网目标，并拒绝发送敏感 header/读取 env secrets 用于认证；需显式 `with_trust_mode(TrustMode::Trusted)` 覆盖。
- `pm-mcp-kit::Manager` 支持自定义 untrusted 下的 `streamable_http` 出站策略：`with_untrusted_streamable_http_policy(UntrustedStreamableHttpPolicy)`。
- `mcpctl` 默认不信任本地配置：本地 stdio/unix 或需要读取 env secrets 的远程 server 需要 `--trust`。
- `pm-jsonrpc` 增加 DoS 防护：限制单条消息大小并使用有界队列缓存 server→client 的 requests/notifications。
