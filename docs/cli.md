...
- `--no-dns-check`：当设置了 `--allow-host` 时，显式关闭默认启用的 DNS 校验（更不安全）
...
- `--allow-host <host>`：Untrusted 下设置 host allowlist（可重复；默认会启用 DNS 校验）

> `--allow-*` / `--dns-check` 只影响 `transport=streamable_http`，不会放开 stdio/unix（它们需要 `--trust --yes-trust`）。
>
